...
### Note: Added sigma2 to function call on 11/28/23

generate_data <- function(N=N, pNP_target=pNP_target, pTheta_target=pTheta_target, pNP2_target=pNP2_target, pTheta2_target=pTheta2_target, Pa=Pa, Pb=Pb, Pa1=Pa1, Pb1=Pb1, 
                          muA=muA, muB=muB, muAC=muAC, muAD=muAD, muBC=muBC, muBD=muBD, omega=omega, delta1=delta1, delta2=delta2, sigma2=sigma2){
  ...
  ## Instead of specifying pathway means, specify muA, muB, muAC, muAD, muBC, and muBD
  ## Also specify delta1, delta2, and omega
  muA <- muA
  muB <- muB
  muAC <- muAC
  muAD <- muAD
  ...
  # First, draw from random effects distribution
  # one random effect for each path
  # tau = 1
  theta <- rep(NA, 20)
  theta[1:4] <- rnorm(4,0,1) #responder theta
  theta[c(5:6,10:11,15:16,18:19)] <- rnorm(8, 1, 1) #theta cluster 1
  theta[c(7:8,9,12,13:14,17,20)] <- rnorm(8, -1, 1) #theta cluster 2
  
  # Second, draw Y | theta
  Y<-rep(NA,N) # 0-no response to treatment, 1-responds to treatment
  
  # response paths
  Y[S1_P == 0 & T1=="A" & R==1]<-rnorm(sum(S1_P == 0 & T1=="A" & R==1), muA + theta[1], sqrt(sigma2))
  Y[S1_P == 1 & T1=="A" & R==1]<-rnorm(sum(S1_P == 1 & T1=="A" & R==1), muA + omega + theta[2], sqrt(sigma2))
  Y[S1_P == 0 & T1=="B" & R==1]<-rnorm(sum(S1_P == 0  & T1=="B" & R==1), muB + theta[3], sqrt(sigma2))
  Y[S1_P == 1 & T1=="B" & R==1]<-rnorm(sum(S1_P == 1 & T1=="B" & R==1), muB + omega + theta[4], sqrt(sigma2))
  
  # AC paths
  Y[S1_P == 0 & T1=="A" & R==0 & S2_P == 0 & T2 == "C"]<-rnorm(sum(S1_P == 0 & T1=="A" & R==0 & S2_P == 0 & T2 == "C"), muAC + theta[5], sqrt(sigma2))
  Y[S1_P == 0 & T1=="A" & R==0 & S2_P == 1 & T2 == "C"]<-rnorm(sum(S1_P == 0 & T1=="A" & R==0 & S2_P == 1 & T2 == "C"), muAC + delta2 + theta[6], sqrt(sigma2))
  Y[S1_P == 1 & T1=="A" & R==0 & S2_P == 0 & T2 == "C"]<-rnorm(sum(S1_P == 1 & T1=="A" & R==0 & S2_P == 0 & T2 == "C"), muAC + delta1 + theta[7], sqrt(sigma2))
  Y[S1_P == 1 & T1=="A" & R==0 & S2_P == 1 & T2 == "C"]<-rnorm(sum(S1_P == 1 & T1=="A" & R==0 & S2_P == 1 & T2 == "C"), muAC + delta1 + delta2 + theta[8], sqrt(sigma2))
  
  # AD paths
  Y[S1_P == 0 & T1=="A" & R==0 & S2_P == 0 & T2 == "D"]<-rnorm(sum(S1_P == 0 & T1=="A" & R==0 & S2_P == 0 & T2 == "D"), muAD + theta[9], sqrt(sigma2))
  Y[S1_P == 0 & T1=="A" & R==0 & S2_P == 1 & T2 == "D"]<-rnorm(sum(S1_P == 0 & T1=="A" & R==0 & S2_P == 1 & T2 == "D"), muAD + delta2 + theta[10], sqrt(sigma2))
  Y[S1_P == 1 & T1=="A" & R==0 & S2_P == 0 & T2 == "D"]<-rnorm(sum(S1_P == 1 & T1=="A" & R==0 & S2_P == 0 & T2 == "D"), muAD + delta1 + theta[11], sqrt(sigma2))
  Y[S1_P == 1 & T1=="A" & R==0 & S2_P == 1 & T2 == "D"]<-rnorm(sum(S1_P == 1 & T1=="A" & R==0 & S2_P == 1 & T2 == "D"), muAD + delta1 + delta2 + theta[12], sqrt(sigma2))
  
  # BC paths
  Y[S1_P == 0 & T1=="B" & R==0 & S2_P == 0 & T2 == "C"]<-rnorm(sum(S1_P == 0 & T1=="B" & R==0 & S2_P == 0 & T2 == "C"), muBC + theta[13], sqrt(sigma2))
  Y[S1_P == 0 & T1=="B" & R==0 & S2_P == 1 & T2 == "C"]<-rnorm(sum(S1_P == 0 & T1=="B" & R==0 & S2_P == 1 & T2 == "C"), muBC + delta2 + theta[14], sqrt(sigma2))
  Y[S1_P == 1 & T1=="B" & R==0 & S2_P == 0 & T2 == "C"]<-rnorm(sum(S1_P == 1 & T1=="B" & R==0 & S2_P == 0 & T2 == "C"), muBC + delta1 + theta[15], sqrt(sigma2))
  Y[S1_P == 1 & T1=="B" & R==0 & S2_P == 1 & T2 == "C"]<-rnorm(sum(S1_P == 1 & T1=="B" & R==0 & S2_P == 1 & T2 == "C"), muBC + delta1 + delta2 + theta[16], sqrt(sigma2))
  
  # BD paths
  Y[S1_P == 0 & T1=="B" & R==0 & S2_P == 0 & T2 == "D"]<-rnorm(sum(S1_P == 0 & T1=="B" & R==0 & S2_P == 0 & T2 == "D"), muBD + theta[17], sqrt(sigma2))
  Y[S1_P == 0 & T1=="B" & R==0 & S2_P == 1 & T2 == "D"]<-rnorm(sum(S1_P == 0 & T1=="B" & R==0 & S2_P == 1 & T2 == "D"), muBD + delta2 + theta[18], sqrt(sigma2))
  Y[S1_P == 1 & T1=="B" & R==0 & S2_P == 0 & T2 == "D"]<-rnorm(sum(S1_P == 1 & T1=="B" & R==0 & S2_P == 0 & T2 == "D"), muBD + delta1 + theta[19], sqrt(sigma2))
  Y[S1_P == 1 & T1=="B" & R==0 & S2_P == 1 & T2 == "D"]<-rnorm(sum(S1_P == 1 & T1=="B" & R==0 & S2_P == 1 & T2 == "D"), muBD + delta1 + delta2 + theta[20], sqrt(sigma2))
  ...
  #data_output_relabel$wtrim <- trim(data_output_relabel$w, at = 0.95)
  #data_output_relabel$wtrimnorm <- (data_output_relabel$wtrim / sum(data_output_relabel$wtrim))*nrow(data_output_relabel)
  
  # normalize weights to sample size
  data_output_relabel$wnorm <- (data_output_relabel$w / sum(data_output_relabel$w))*nrow(data_output_relabel)
  ...
  replicated_dat$repwnormn <- (replicated_dat$w / sum(replicated_dat$w))*(nrow(data_output_relabel))
  
  replicated_dat$T1_A = as.numeric(replicated_dat$T1 == 1)
  replicated_dat$T1_B = as.numeric(replicated_dat$T1 == 0)
  replicated_dat$T2_C = as.numeric(replicated_dat$T2 == 1)
  replicated_dat$T2_D = as.numeric(replicated_dat$T2 == 0)
