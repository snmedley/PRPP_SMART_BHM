## Runs more efficient version of the EXNEX BUGS code
...
data_full = c(list(
    ...
    c(m_A_0, m_B_0, m_AC_00, m_AC_00, m_BC_00, m_BD_00), w, nu,
    # RESPONSE DATA
...
jag <- rjags::jags.model(
    file = paste0("EXNEX_Efficient.bugs"),
    data = data_full,
...
R_hat = coda::gelman.diag(posterior_full[,21:45])$psrf[,1]
n_eff = coda::effectiveSize(posterior_full[,21:45])
param_hat = apply(draws_full[,c(1:22,39:45)], 2, mean)
param_var = apply(draws_full[,c(1:22,39:45)], 2, var)
DTR_hat = apply(draws_full[,23:38], 2, mean)
DTR_var = apply(draws_full[,23:38], 2, var)
dtr_quantiles = cbind(apply(draws_full[,23:38], 2, function(x) quantile(x, probs = 0.025)), 
                      apply(draws_full[,23:38], 2, function(x) quantile(x, probs = 0.975)))
